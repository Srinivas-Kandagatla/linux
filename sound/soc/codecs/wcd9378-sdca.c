 ...
 * ASL is supplied via populate_function.
 */
...
/**
 * struct wcd9378_priv - per-slave codec state
 * @class: sdca_class_drv handed to the SDCA class helper.  Embedded
 *         first so container_of() from &class -> &priv works if we
 *         ever need it, though today the class code never dereferences
 *         its way back into the codec priv.
 * @micbias2_val: DT-derived MICB2 rail encoded as an IT33 mic-bias
 *         selector index (per SDCA MicBias range).  Written into the
 *         IT33 mic-bias register from pde_pre_pmu.  Per-slave so a
 *         board with two WCD9378 instances can use different MICB2
 *         voltages.
 */
struct wcd9378_priv {
...
/*
 * ctrl_it33_micbias_vals is a template default only; the actual
 * per-slave MICB2 index is stored in struct wcd9378_priv.micbias2_val
 * and written to the mic-bias register from pde_pre_pmu.
 */
static int ctrl_it33_micbias_vals[] = { 0x5 };	/* 2.75V mic bias (template) */
					  ...
					  struct sdca_function_data *function)
{
	 ...
	 * descriptor allocated by sdca_dev_register_functions(); leave it
	 * alone and only copy the templated payload (entities, clusters,
	 * init_table, delays).  WCD9378 has a single SimpleJack function.
	 */
				      ...
				      unsigned int entity_id)
{
	...
	if (entity_id == WCD9378_PDE34_ENTITY_ID) {
		...
		ret = regmap_write(regmap, mic_bias, priv->micbias2_val);
		if (ret)
				    ...
				    const struct sdw_device_id *id)
{
	...
	/*
	 * On DT/ARM there is no DisCo/ACPI enumeration, so sdca_data stays
	 * empty and sdca_dev_register_functions() would create no auxdevs.
	 * Seed the short function descriptor from the static topology; the
	 * per-function payload (entities/clusters/init_table/...) is filled
	 * later by wcd9378_sdca_populate_function() via hw_ops.  The guard
	 * keeps this a no-op on the DisCo path in case that ever runs first.
	 */
...
static void wcd9378_sdca_sdw_remove(struct sdw_slave *slave)
{
	struct wcd9378_priv *priv = dev_get_drvdata(&slave->dev);
...
/*
 * Codec-owned PM ops: the class helpers take a struct sdca_class_drv *,
 * so we compose them here after fetching our priv from drvdata.
 */
static int wcd9378_sdca_runtime_suspend(struct device *dev)
{
	struct wcd9378_priv *priv = dev_get_drvdata(dev);
...
static int wcd9378_sdca_runtime_resume(struct device *dev)
{
	struct wcd9378_priv *priv = dev_get_drvdata(dev);
...
static int wcd9378_sdca_system_suspend(struct device *dev)
{
	struct wcd9378_priv *priv = dev_get_drvdata(dev);
...
static int wcd9378_sdca_system_resume(struct device *dev)
{
	struct wcd9378_priv *priv = dev_get_drvdata(dev);
...
static const struct dev_pm_ops wcd9378_sdca_pm_ops = {
	SYSTEM_SLEEP_PM_OPS(wcd9378_sdca_system_suspend, wcd9378_sdca_system_resume)
	RUNTIME_PM_OPS(wcd9378_sdca_runtime_suspend, wcd9378_sdca_runtime_resume, NULL)
};
...
static const struct sdw_device_id wcd9378_sdca_sdw_id[] = {
	SDW_SLAVE_ENTRY(0x0217, 0x0110, &wcd9378_sdca_hw_ops),
	{}
};
...
static struct sdw_driver wcd9378_sdca_sdw_driver = {
	.driver = {
		.name	= "wcd9378-sdca",
		.pm	= pm_ptr(&wcd9378_sdca_pm_ops),
	},
	.probe		= wcd9378_sdca_sdw_probe,
	.remove		= wcd9378_sdca_sdw_remove,
